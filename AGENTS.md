...
## Pre-commit проверка документации

Перед каждым коммитом:

1. Прочитать `AGENTS.md` и `README.md`
2. Проверить актуальность: соответствуют ли текущей структуре проекта, JSON-схемам, списку скриптов, workflows
3. При необходимости внести правки (обновить устаревшие схемы, добавить новые скрипты, поправить описание структуры)
4. Проверить неотслеживаемые файлы (`git status`):
   - Файлы, которые должны войти в коммит (новые JSON-уроки, тесты, скрипты) — `git add`
   - Заведомо ненужные (временные логи, черновики, дампы) — не добавлять
   - Временные/мусорные файлы — удалить
5. Написать лаконичный commit message на английском языке, соответствующий стилю репозитория и описывающий суть изменений
6. Только после этого выполнять коммит
